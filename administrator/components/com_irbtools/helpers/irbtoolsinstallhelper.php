<?php
   ...
   * @since 1.5
   */
class JHTMLIrbtoolsInstallHelper
{
	function getVersionLocal()
	{
          // Código extraido de PhocaGallery
		$folder = JPATH_ADMINISTRATOR . DS . 'components' . DS . 'com_irbtools';
      ...
      function getVersionRemote()
      {
          ...
          $url = $params->get('irbtoolsConfig_InstallUrl') . 'administrator/components/com_irbtools/irbtools.xml';
      ...
      function _sql($sql)
      {
          ...
          $sql_file = JPATH_SITE . DS . 'administrator' . DS . 'components' . DS . 'com_irbtools' . DS . 'sql' . DS . $sql . '.sql';
      ...
      {
          if ($local != null) {
              $files = JFolder::files(JPATH_SITE . DS . 'administrator' . DS . 'components' . DS . 'com_irbtools' . DS . 'sql');
              foreach ($files as $f) {
                  if (substr($f, 12, 1) == "") {
                      $actual = substr($f, 0, 8);
                      if (($actual > $local) && ($actual <= $remote)) {
                          JHTMLIrbtoolsInstallHelper::_sql($actual);
                      }
                  }
              }
          } else {
